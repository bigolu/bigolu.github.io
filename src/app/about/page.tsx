...
export default function About() {
  ...
  function makeSkillLists() {
    const skillLists = Object.entries(skills)
      .map(([subject, skills]) => {
        return (
      ...
      .map((fragment, index) => (
        <li className={styles.skillList} key={index}>
          {fragment}
        </li>
      ));

    return <ul className={styles.topLevelLists}>{skillLists}</ul>;
  }
  ...
  return (
    <main className={styles.container}>
      <div className={styles.me}>
        ...
        <div
          className={`nes-container ${styles["greeting-container"]}`}
          style={{ padding: "5px" }}
        >
